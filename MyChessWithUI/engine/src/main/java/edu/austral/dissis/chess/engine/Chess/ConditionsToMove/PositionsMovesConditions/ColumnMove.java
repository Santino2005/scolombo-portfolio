...
public class ColumnMove implements Valid {

    @Override
    public boolean valid(MoveTo moveTo, Board board) {
        int dir = moveTo.getFrom().column();
        return moveTo.getFrom().row() == moveTo.getTo().row() && (dir > moveTo.getTo().column() || dir < moveTo.getTo().column());
    }
}
